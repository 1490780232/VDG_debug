...
def main_worker(args):
    ...
    aug_loader = get_augset_loader(dataset,  args.height, args.width, args.batch_size, args.workers,"/home/lzy/VDG/market_train2")
    ...
    # Trainer
    trainer = VDGTrainer_USL(model, memory)
    ...
    for epoch in range(args.epochs):
        ...
        features, _ = extract_features(model, cluster_loader, print_freq=50)
        features_cal = torch.cat([features[f].unsqueeze(0) for f, _, _ in sorted(dataset.train)], 0)
        rerank_dist = compute_jaccard_distance(features_cal, k1=args.k1, k2=args.k2)
        features_aug, _ ,_ = extract_aug_features(model, aug_loader,print_freq=50)
        for f, _,_ in sorted(dataset.train):
                f_augfeatures = []
                # # print(f)
                # key = f.split("/")[-1][:-4]
        ...
        features = torch.cat([features[f].unsqueeze(0) for f, _, _ in sorted(dataset.train)], 0)
        rerank_dist_aug = compute_jaccard_distance(features, k1=args.k1, k2=args.k2)
        # del features
        if (epoch==0):
            # DBSCAN cluster
            eps = args.eps
        ...
        pseudo_labels_aug = cluster.fit_predict(rerank_dist_aug)
        # pseudo_labels = pseudo_labels_aug
        ...
        num_ids_aug = len(set(pseudo_labels_aug)) - (1 if -1 in pseudo_labels_aug else 0)
        ...
        pseudo_labels_aug = generate_pseudo_labels(pseudo_labels_aug, num_ids_aug)
        ...
        R_comp = 1-torch.min(label_sim, label_sim_aug).sum(-1)/torch.max(label_sim, label_sim_aug).sum(-1)
        ...
        outliers = 0
        # pseudo_labels_refine = pseudo_labels.copy()
        ...
        for i, label in enumerate(pseudo_labels):
            # indep_score = cluster_R_indep[label.item()]
            comp_score = R_comp[i] #((indep_score<=indep_thres) and (cluster_R_comp[label.item()]))
            if comp_score.item()<=0.6:
                # pseudo_labeled_dataset.append((fname,label.item(),cid))
                continue
            else:
                count+=1
                pseudo_labels[i] = -1
        print("count_num_samples", count)
        ...
        for i, ((fname, _, cid), label) in enumerate(zip(sorted(dataset.train), pseudo_labels)):
            ...
            #     outliers+=1
        print("datasetlen", len(pseudo_labeled_dataset))
        ...
        print(pseudo_labels.shape, centroids.shape)
