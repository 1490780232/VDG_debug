...
def extract_aug_features(model, data_loader, print_freq=50):
    model.eval()
    ...
    with torch.no_grad():
        for i, (imgs, fnames, pids, camid, _) in enumerate(data_loader):
            ...
            for fname, output, pid,camid in zip(fnames, outputs, pids,camid):
                features[pid] = output
                labels[pid] = pid
                cam_id[pid]=camid
...
def extract_features_view(model, data_loader, print_freq=50):
    model.eval()
    batch_time = AverageMeter()
    data_time = AverageMeter()
    ...
    with torch.no_grad():
        for i, (imgs, fnames, pids, viewids, _, _) in enumerate(data_loader):
            data_time.update(time.time() - end)

            outputs = extract_cnn_feature(model, imgs)
            for fname, output, pid, viewid in zip(fnames, outputs, pids, viewids):
                features[fname] = output
                labels[fname] = pid
                views[fname] = viewid
            batch_time.update(time.time() - end)
